...
public class TwoPairsHand extends Hand {
  ...
  public HandCompare compare(Hand otherHand) {
    ...
    if (this.remainingFigure.value > otherTwoPairsHand.remainingFigure.value) {
      return new HandCompare(HandCompare.Result.WIN, this.getWinningCondition(this.pairFigures.get(0), this.pairFigures.get(1), this.remainingFigure));
    } else if (this.remainingFigure.value < otherTwoPairsHand.remainingFigure.value) {
      return new HandCompare(HandCompare.Result.LOSE, this.getWinningCondition(otherTwoPairsHand.pairFigures.get(0), otherTwoPairsHand.pairFigures.get(1), otherTwoPairsHand.remainingFigure));
    }
